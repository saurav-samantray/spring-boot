...
public class L7QNMinimizer implements Minimizer<DiffFunction>, HasEvaluators  {
  ...
  public L7QNMinimizer() {
    this((Function) null);
  }

  public L7QNMinimizer(int m) {
    this(null, m);
  }

  public L7QNMinimizer(int m, boolean useRobustOptions) {
    this(null, m, useRobustOptions);
  }

  public L7QNMinimizer(Function monitor) {
    this.monitor = monitor;
  }

  public L7QNMinimizer(Function monitor, int m) {
    this(monitor, m, false);
  }

  public L7QNMinimizer(Function monitor, int m, boolean useRobustOptions) {
    this.monitor = monitor;
    mem = m;
    if (useRobustOptions) {
  ...
  public L7QNMinimizer(FloatFunction monitor) {
  ...
  public double[] minimize(DiffFunction dFunction, double functionTolerance,
    ...
    if (outputToFile) {
      try {
    	String PATH = "output/"+jobId+"/";
    	File directory = new File(PATH);
        if (! directory.exists()){
            directory.mkdir();
        }
    	  
        String baseName = PATH+"QN_m" + mem + '_' + lsOpt.toString() + '_'
            + scaleOpt.toString();
